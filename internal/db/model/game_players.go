...
type GamePlayer struct {
	...
	Profit   decimal.Decimal `boil:"profit" json:"profit" toml:"profit" yaml:"profit"`
	Fee      decimal.Decimal `boil:"fee" json:"fee" toml:"fee" yaml:"fee"`
	GaveUp   bool            `boil:"gave_up" json:"gave_up" toml:"gave_up" yaml:"gave_up"`
	Draw     bool            `boil:"draw" json:"draw" toml:"draw" yaml:"draw"`
...
var GamePlayerTableColumns = struct {
	...
	GameID:   "game_players.game_id",
	PlayerID: "game_players.player_id",
	Position: "game_players.position",
	Profit:   "game_players.profit",
	Fee:      "game_players.fee",
	GaveUp:   "game_players.gave_up",
	Draw:     "game_players.draw",
}
...
var GamePlayerWhere = struct {
	...
	Profit   whereHelperdecimal_Decimal
	Fee      whereHelperdecimal_Decimal
	GaveUp   whereHelperbool
	Draw     whereHelperbool
}{
	GameID:   whereHelperstring{field: "\"kgames\".\"game_players\".\"game_id\""},
	PlayerID: whereHelperstring{field: "\"kgames\".\"game_players\".\"player_id\""},
	Position: whereHelperint16{field: "\"kgames\".\"game_players\".\"position\""},
	Profit:   whereHelperdecimal_Decimal{field: "\"kgames\".\"game_players\".\"profit\""},
	Fee:      whereHelperdecimal_Decimal{field: "\"kgames\".\"game_players\".\"fee\""},
	GaveUp:   whereHelperbool{field: "\"kgames\".\"game_players\".\"gave_up\""},
	Draw:     whereHelperbool{field: "\"kgames\".\"game_players\".\"draw\""},
}
...
type gamePlayerR struct {
	...
	Player *User `boil:"Player" json:"Player" toml:"Player" yaml:"Player"`
}
...
func (r *gamePlayerR) GetPlayer() *User {
	if r == nil {
		return nil
	}
	return r.Player
...
var (
	gamePlayerAllColumns            = []string{"game_id", "player_id", "position", "profit", "fee", "gave_up", "draw"}
	gamePlayerColumnsWithoutDefault = []string{"game_id", "player_id", "position"}
	gamePlayerColumnsWithDefault    = []string{"profit", "fee", "gave_up", "draw"}
	gamePlayerPrimaryKeyColumns     = []string{"game_id", "player_id"}
...
func (q gamePlayerQuery) One(ctx context.Context, exec boil.ContextExecutor) (*GamePlayer, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: failed to execute a one query for game_players")
	}
...
func (q gamePlayerQuery) All(ctx context.Context, exec boil.ContextExecutor) (GamePlayerSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "model: failed to assign all query results to GamePlayer slice")
	}
...
func (q gamePlayerQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to count game_players rows")
	}
...
func (q gamePlayerQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "model: failed to check if game_players exists")
	}
...
func (o *GamePlayer) Player(mods ...qm.QueryMod) userQuery {
	queryMods := []qm.QueryMod{
...
// loaded structs of the objects. This is for an N-1 relationship.
func (gamePlayerL) LoadPlayer(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGamePlayer interface{}, mods queries.Applicator) error {
	var slice []*GamePlayer
	var object *GamePlayer
	...
	if singular {
		...
		foreign.R.PlayerGamePlayers = append(foreign.R.PlayerGamePlayers, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.PlayerID == foreign.ID {
				...
				foreign.R.PlayerGamePlayers = append(foreign.R.PlayerGamePlayers, local)
				break
			}
		}
	}
...
func (o *GamePlayer) SetGame(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Game) error {
	...
	values := []interface{}{related.ID, o.GameID, o.PlayerID}
...
// SetPlayer of the gamePlayer to the related item.
// Sets o.R.Player to related.
// Adds o to related.R.PlayerGamePlayers.
func (o *GamePlayer) SetPlayer(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &userR{
			PlayerGamePlayers: GamePlayerSlice{o},
		}
	} else {
		related.R.PlayerGamePlayers = append(related.R.PlayerGamePlayers, o)
	}
...
func GamePlayers(mods ...qm.QueryMod) gamePlayerQuery {
	mods = append(mods, qm.From("\"kgames\".\"game_players\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"kgames\".\"game_players\".*"})
	}
...
func FindGamePlayer(ctx context.Context, exec boil.ContextExecutor, gameID string, playerID string, selectCols ...string) (*GamePlayer, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"kgames\".\"game_players\" where \"game_id\"=$1 AND \"player_id\"=$2", sel,
	)

	q := queries.Raw(query, gameID, playerID)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: unable to select from game_players")
	}
...
func (o *GamePlayer) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("model: no game_players provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"kgames\".\"game_players\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"kgames\".\"game_players\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to insert into game_players")
	}
...
func (o *GamePlayer) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("model: unable to update game_players, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"kgames\".\"game_players\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update game_players row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by update for game_players")
	}
...
func (q gamePlayerQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update all for game_players")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected for game_players")
	}
...
func (o GamePlayerSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"kgames\".\"game_players\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update all in gamePlayer slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected all in update all gamePlayer")
	}
	return rowsAff, nil
}
...
func (o *GamePlayer) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
		return errors.New("model: no game_players provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("model: unable to upsert game_players, could not build update column list")
		}
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(gamePlayerPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert game_players, could not build conflict column list")
			}

			conflict = make([]string, len(gamePlayerPrimaryKeyColumns))
			copy(conflict, gamePlayerPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"kgames\".\"game_players\"", updateOnConflict, ret, update, conflict, insert, opts...)
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to upsert game_players")
	}
...
func (o *GamePlayer) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("model: no GamePlayer provided for delete")
	}
	...
	sql := "DELETE FROM \"kgames\".\"game_players\" WHERE \"game_id\"=$1 AND \"player_id\"=$2"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete from game_players")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by delete for game_players")
	}
...
func (q gamePlayerQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("model: no gamePlayerQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete all from game_players")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by deleteall for game_players")
	}
...
func (o GamePlayerSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"kgames\".\"game_players\" WHERE " +
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete all from gamePlayer slice")
	}
...
func (o *GamePlayer) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindGamePlayer(ctx, exec, o.GameID, o.PlayerID)
	if err != nil {
		return err
	}
...
func (o *GamePlayerSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"kgames\".\"game_players\".* FROM \"kgames\".\"game_players\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, gamePlayerPrimaryKeyColumns, len(*o))
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to reload all in GamePlayerSlice")
	}
...
func GamePlayerExists(ctx context.Context, exec boil.ContextExecutor, gameID string, playerID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"kgames\".\"game_players\" where \"game_id\"=$1 AND \"player_id\"=$2 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, gameID, playerID)
	}
	row := exec.QueryRowContext(ctx, sql, gameID, playerID)

	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "model: unable to check if game_players exists")
	}
...
func (o *GamePlayer) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return GamePlayerExists(ctx, exec, o.GameID, o.PlayerID)
}
